...
export const SystemQr = ({ id }: { id: number }) => {
  const [p] = useTranslation('panel')

  const { register, control } = useFormContext()

  const onChangeImage = async (cb: any, file: any) => {
    const nameFile = file[0]?.name
    const image = await toBase64(file[0])
    await cb({
      image,
      name: nameFile,
    })
    ...
    <div className={s.qrBlock}>
      <Controller
        render={({ field: { onChange, value }, formState: { errors } }) => (
          <label className={s.input} htmlFor={`qrCode_${id}`}>
            <input
              {...register(`qrCode_${id}`)}
              type="file"
              id={`qrCode_${id}`}
              accept="image/png, image/jpeg, image/pdf"
              onChange={(e) => onChangeImage(onChange, e.currentTarget.files)}
            />
            <svg
            ...
            {value?.name ? (
              <div className={s.file}>{value.name.toString()}</div>
            ) : (
